# 薯条菌网站（Astro）

一个使用 Astro 构建的静态网站，包含三大栏目：

- 原味薯条
- 蕃茄酱
- 海鸥看过

网站已接入云端互动（点赞 + 评论 + 阅读总数）：

- 用户先登记 `用户名 + 联系邮箱`
- 点赞与评论数据写入 Supabase
- 阅读总数按浏览器去重后写入 Supabase
- 评论区邮箱默认脱敏显示

## 本地开发

```bash
npm install
npm run dev
```

## 生产构建

```bash
npm run build
```

## 互动功能配置（Supabase）

1. 在 Supabase 新建项目。
2. 打开 SQL Editor，执行 [`supabase/engagement.sql`](./supabase/engagement.sql)。
   已经执行过旧版 SQL 的话，也请重新执行一遍，新增了 `post_views` 表用于阅读总数。
3. 复制 `.env.example` 为 `.env`，填入：

```bash
PUBLIC_SUPABASE_URL=https://YOUR_PROJECT_ID.supabase.co
PUBLIC_SUPABASE_ANON_KEY=YOUR_SUPABASE_ANON_KEY
```

4. 重新构建并部署。

如果未配置这两个环境变量，页面会显示“互动云端未配置”，并自动禁用点赞/评论按钮。

## 技术栈

- Astro 5
- Content Collections
- Supabase REST API（无后端服务，纯静态站可用）

## 项目结构（已整理）

- `src/pages/`：页面路由（首页、栏目页、文章页）
- `src/components/`：可复用组件（含顶部标头、侧栏、互动面板）
- `src/layouts/`：文章布局
- `src/content/articles/`：文章内容（Markdown）
- `src/lib/`：栏目与内容数据处理
- `src/styles/`：全局样式
- `public/`：静态资源
- `supabase/engagement.sql`：互动数据表 SQL

## 后续新增栏目（建议流程）

1. 在 `src/lib/columns.ts` 新增栏目元数据（`slug/name/summary`）。
2. 在 `src/content/articles/` 新增该栏目文章，并设置 `column` 字段。
3. 检查页面展示：
   - 首页聚合
   - `/<column>/` 栏目汇总页
   - `/<column>/<slug>/` 文章详情页
4. 本地验证：`npm run build`
